...
public class GlassService extends Service {
	...
	private static final String BASH_URL = "http://where-is-my-child.herokuapp.com/last_coordinate";
    ...
    //public static final String API_KEY = "AIzaSyDVYDfQkIpCS4n5ZUrxPg3can_8yedOVD8";
...
//						.setAction(MenuItem.Action.SHARE).build();
				MenuItem customOption = MenuItem.newBuilder()
						.addValue(MenuValue.newBuilder()
								.setDisplayName("Where Is My Child?")
								.build())
...
//						.addMenuItem(share)
	...
	private UkrBashPicture requestAndParse() throws Exception {
        ...
        String splitResp[] = responseStr.split(",");
        String lat = splitResp[0];
        String lon = splitResp[1];


        String googleMapUrl = "https://maps.googleapis.com/maps/api/staticmap?center=" + lat + "," + lon + "&format=jpg&zoom=12&size=400x400&markers=" + lat + "," + lon;
        ...
        return new UkrBashPicture(googleMapUrl, itemText, translation);
	}
